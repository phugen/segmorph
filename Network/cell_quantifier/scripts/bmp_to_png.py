...
if IMG_NO == 0:
    print "No BMP images found in path" + path + "!"
    sys.exit(-1)
...
for filename in all_images:
    if not filename.endswith("_label.bmp"): # treat label images inside the loop
        with Image.open(filename) as bmp_img, Image.open(filename[:-4] + "_label.bmp") as bmp_label:
            ...
            # resize to "magic dimensions" 596H x 948W given by data dims and U-Net conditions
            bmp_img = bmp_img.resize((420, 420))
            bmp_label = bmp_label.resize((420, 420))

            bmp_img.save(filename[:-4] + ".png", "PNG")
            bmp_label.save(filename[:-4] + "_label.png", "PNG")

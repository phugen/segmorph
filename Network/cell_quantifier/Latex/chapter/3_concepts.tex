...
\begin {align}
	\mu_0 &= \frac{1}{w_0} \sum \limits_{i=1}^{\theta} i \cdot p_i \\
	\mu_1 &= \frac{1}{w_1} \sum \limits_{i=\theta + 1}^{k} i \cdot p_i\\
\end {align}
...
\textit{Gradient Descent} is a standard, numeric optimization technique that iteratively optimizes at least once-differentiable, continuous functions, that is, that calculates a vector of parameters $\Theta$ for a function $f$ so that

\[ \Theta = \argmin \limits_{i} f(i) \]

\noindent To find values for $\Theta$, the approach takes advantage of the fact that the gradient vector 

...
Image segmentation is one of the main challenges in image processing and has been an active field of study for years, which resulted in many segmentation methods being available. In the following pages, a selection of methods that work for multi-class segmentation, i.e. segmentation with more than two classes, is presented. All methods described here are later used to segment a set of images and compared to the ground truth to measure the quality of the segmentations (see \ref{chap:results}.
...
\noindent To extend the normal Otsu algorithm to $n$-class segmentation, the between-class variance formula is changed to include the variances of all classes, so that the best set of thresholds $\theta = \{t_1 \dots t_{n-1}\}$ is the one that minimizes the overall within-class variance by maximizing the overall between-class variance:

...
\begin {figure}[!ht]
		\begin{center}
			\textbf{TODO: Change right figure}
			\input{img/fig_mlp_trick.pgf}
		\end{center}
		\caption[]{\textbf{Left:} The problem from Figure \textbf{\ref{fig:perceptron}} revisited. An 2-layer MLP with 2 input neurons, 5 hidden neurons and 2 output neurons was trained on the dataset for 10,000 iterations using Backpropagation and Gradient Descent. The dataset is classified correctly using a nonlinear classification boundary. \textbf{Right:} Visualization of the transformed data with $(x, y) \Rightarrow (x, y, out(x, y))$. The transformed dataset is then linearly separable by a plane.}
		\label{fig:mlp_trick}
	\end {figure}

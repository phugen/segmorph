...
\afterpage{
	\begin {figure}[!ht]
		...
		\end{center}
		\caption[]{Application of the chain rule in the Backpropagation algorithm\protect\footnotemark. Values calculated during the forward pass are shown in blue, while values calculated during the backward pass are orange. The partial derivatives of the input variables $x$ and $y$ measure the (local) influence on the output of the neuron, $z$. When the influence of $z$ on the loss function $L$ is backpropagated to the neuron, the chain rule is applied to recursively backpropagate the influence of outputs of the previous neurons on $L$, which are the inputs to the depicted neuron. This is repeated throughout the entire network until all partial derivatives are known and a single Gradient Descent step can be taken toward the loss function minimum.}
		\label{fig:mlp}
	\end {figure}

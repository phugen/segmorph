...
\textit{Gradient Descent} or the ``method of steepest descent'' is a standard, analytic first-order technique that can be used to minimize or maximize at least once-differentiable functions, that is, to calculate a vector of parameters $\Theta$ so that
...
The algorithm starts with a guess for the parameters in $\Theta$ and then iteratively evaluates the gradient and updates the parameters accordingly until convergence is reached (see algorithm \ref{algo:grad_desc}).
...
\noindent The normal gradient descent evaluates all samples and updates the parameters with the averaged gradient of all these samples. A variation of the algorithm is the Stochastic Gradient Descent (SGD) which performs updates for each sample individually (see algorithm \ref{algo:sgd}). This has the benefit of not having to load the entire dataset into memory, but can result in strong fluctuations during training. The middle path between both algorithms is to use ''mini-batches``, i.e. performing updates for small subsets of the data each.

\begin {figure}[!ht]
	\begin{center}
		\includegraphics[scale=0.9]{img/fig_grad_desc}
	\end{center}
	\caption{A contour plot of Gradient Descent minimization of $f(a, b) = \sin(a) + \cos^2(b)$. The blue line shows the search path of the algorithm along the negative gradient while estimating the optimal $\Theta$.}
	\label{fig:grad_desc}
\end {figure}
...
\noindent where $\mathcal{M}$ is the set of all data points that were misclassfied, meaning that evaluating equation \ref{eq:perceptron_label} for that data yields a value $< 0$. 

Applying stochastic gradient descent to the perceptron criterion \ref{eq:perc_error} yields an iterative algorithm for finding correct values for $w$ (see algorithm \ref{alg:perceptron_algorithm}). \cite[pp. 192--194]{bishop_pattern}

\begin {algorithm}
	\begin {algorithmic}[1]
		\While {true}
			\State $\Delta = 0$
			\For{all $x_i$ with $wx^t_i \leq 0$}
				\State $\Delta += x_i$
			\EndFor
			\State $w = w + p_k * \Delta$
			\If {for all $x_i$, $wx^t_i > 0$}
				\State stop
			\EndIf
		\EndWhile
	\end{algorithmic}
	\caption{Gradient descent applied to the task of finding the perceptron weights $w$.}

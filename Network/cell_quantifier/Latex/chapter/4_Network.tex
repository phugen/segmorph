...
%  This is especially useful because if one wants to assign exactly one of many classes to an input, using probabilities is not only a human-readable choice, but also makes the training faster. Considering a case where the raw %output of the network is  
%
%\[ out_1 =  5.6,\,\, out_2 = 2.3,\,\, out_3  = 10^{-12} \]
%
%\noindent and $out_3$ is the correct output as given by the label data, the weights must be changed by during training so that $out_3$ becomes larger. %Unfortunately, this will take a long time because the partial derivative will be very small, slowing down the training. Applying the Softmax function to this example yields %new outputs
%
%\[ out_1 =  0.961,\,\, out_2 = 0.0354,\,\, out_3  = 0.0035 \] 
%
%\npindent and thus larger gradients while keeping the relationship between the values intact.


However, the above definition of the Softmax function can become numerically unstable when the exponentiation yields large values because dividing by large numbers could potentially produce zero values due to insufficient floating-point precision. \cite[p. 81]{deeplearning_book} To prevent this, the calculation can be made robust by defining it as

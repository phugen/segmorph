...
Additionally, the network uses feature concatenation by using shortcut connections (called ``skip-layer connections'' in \cite{bishop_pattern}) , taking the non-padded part of the feature maps from the corresponding sampling level after each upsampling operation and fusing them with the upsampled maps by \textbf{TODO: how does fusing work? summation? maximum?}. This is done to boost the precision of the segmentation output - downsampling reduces locality information which is nonetheless needed when creating a segmentation map, so this is a way to reduce the complexity of the network while still achieving precise results.
...
\begin {figure}[!ht]
	...
	\caption{The U-Net architecture. The numbers indicate the number of channels, or, in the case of the convolutional layers, the number of filters. In this example, the input is a grayscale image and therefore has one channel only. $C$ is the number of output classes to segment the image into, i.e. 3 or 4 depending on which dataset is used.}
	\label{fig:unet_arch}
\end {figure}

    ...
    def backward(self, top, propagate_down, bottom):
        for btm in [0]:
            ...
            for i in range(0, bottom[btm].diff.shape[0]):
                ...
                denom = (self.union[i]) ** 2
                partials = 2.0 * (self.gt[i, :] * self.union[i] - self.intersection[i]) / denom
                bottom[btm].diff[i, 0, :] += partials
                bottom[btm].diff[i, 1, :] -= partials

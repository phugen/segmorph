...
class F1Loss(caffe.Layer):
    ...
    def forward(self, bottom, top):
        ...
        self.oneHotLabels = np.eye(self.numLabels)[bottom[1].data.astype(int)] \
                            .reshape(numBatches, bottom[0].data.shape[2] * bottom[0].data.shape[3], bottom[0].data.shape[1]) # 2D label input -> 1D
        ...
        result = np.reshape(np.squeeze(np.argmax(bottom[0].data[...],axis=1)),[numBatches,bottom[0].data.shape[2] * bottom[0].data.shape[3]])

        # create the one-hot encoding for this
        oneHotResult = np.eye(self.numLabels)[result.astype(dtype=np.uint32)]
        #print oneHotResult.shape
        ...
        union = np.atleast_2d(np.zeros([numBatches],dtype=np.float32))
        intersection = np.atleast_2d(np.zeros([numBatches],dtype=np.float32))

        for c in range(0, self.numLabels):
            # get the binary results just for this class
            classResult = np.atleast_2d(np.squeeze(oneHotResult[:, :, c]).astype(dtype=np.float32))
            # get the binary labels just for this class
            classLabels = np.atleast_2d(np.squeeze(self.oneHotLabels[:, :, c]).astype(dtype=np.float32))
    ...
    def backward(self, top, propagate_down, bottom):
        for btm in [0]:
            ...
            for c in range(0, self.numLabels):
                ...
                currentClassProb = np.reshape(np.squeeze(bottom[btm].data[:,c,:]), [numBatches, bottom[0].data.shape[2] * bottom[0].data.shape[3]]).astype(dtype=np.float32)
                classLabels = np.atleast_2d(np.squeeze(self.oneHotLabels[:, :, c]).astype(dtype=np.float32))
                ...
                for i in range(0, bottom[btm].diff.shape[0]):
                    ...
                    denom = (union[i]) ** 2
                    partials = np.reshape(2.0 * (classLabels[i, :] * union[i] - intersection[i]) / denom, (bottom[0].data.shape[2], bottom[0].data.shape[3]))
